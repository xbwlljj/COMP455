...
# To make sure the Cifar-10 dataset in the folder
def test_folder_path(cifar10_dataset_folder_path):
    ...
    assert cifar10_dataset_folder_path[-1] != '/',\
        'The  end of the path shouldn\'t add "/"'
    ...
    assert not missing_files,\
        'Missing files in: {}'.format(missing_files)
...
# To make sure the normal function that is used in main project is working correctly.
def test_normal(normal):
    test_shape = (np.random.choice(range(1000)), 32, 32, 3)
    test_numbers = np.random.choice(range(256), test_shape)
    normal_out = normal(test_numbers)

    assert type(normal_out).__module__ == np.__name__,\
        'Not Numpy Object'

    assert normal_out.shape == test_shape,\
        'Incorrect Shape. {} shape found'.format(normal_out.shape)

    assert normal_out.max() <= 1 and normal_out.min() >= 0,\
        'Incorect Range. {} to {} found'.format(normal_out.min(), normal_out.max())

    _print_success_message()

# To make sure the one_hot_encode function that is used in main project is working correctly.
...
# To make sure the neural_image_inputs function that is used in main project is working correctly.
...
# To make sure the neural_label_inputs function that is used in main project is working correctly.
...
# To make sure the neural_keep_prob function that is used in main project is working correctly.
def test_nn_keep_inputs(neural_net_keep_prob_input):
    ...
    assert nn_inputs_out_k.op.type == 'Placeholder',\
        'Type is INCORRECT.  Found {} type'.format(nn_inputs_out_k.op.type)

    assert nn_inputs_out_k.name == 'keep_prob:0', \
        'Name is INCORRECT.  Found {}'.format(nn_inputs_out_k.name)

    print('Keep Prob Tests Passed.')


# To make sure the "output" function that is used in main project is working correctly.
def test_output(output):
    ...
    assert output_out.get_shape().as_list() == [None, 40],\
        'Shape is INCORRECT.  Found {} shape'.format(output_out.get_shape().as_list())

    _print_success_message()


# To make sure the "convolution_maxpool" function that is used in main project is working correctly.
def test_con_pool(convolution_maxpool):
    ...
    conv2d_maxpool_out = convolution_maxpool(test_x, test_num_outputs, test_con_k, test_con_s, test_pool_k, test_pool_s)

    assert conv2d_maxpool_out.get_shape().as_list() == [None, 4, 4, 10],\
        'Shape is INCORRECT.  Found {} shape'.format(conv2d_maxpool_out.get_shape().as_list())

    _print_success_message()


# To make sure the "flat" function that is used in main project is working correctly.
def test_flat(flat):
    test_x = tf.placeholder(tf.float32, [None, 10, 30, 6])
    flat_out = flat(test_x)

    assert flat_out.get_shape().as_list() == [None, 10*30*6],\
        'Shape is INCORRECT.  Found {} shape'.format(flat_out.get_shape().as_list())

    _print_success_message()


# To make sure the "conv_net" function that is used in main project is working correctly.
def test_conv_net(conv_net):
    ...
    assert logits_out.get_shape().as_list() == [None, 10],\
        'Output is INCORRECT.  Found {}'.format(logits_out.get_shape().as_list())

    print('Neural Network has been Built!')


# To make sure the "fully_conn" function that is used in main project is working correctly.
def test_fully_conn(fully_conn):
    ...
    assert fc_out.get_shape().as_list() == [None, 40],\
        'Shape is INCORRECT.  Found {} shape'.format(fc_out.get_shape().as_list())

    _print_success_message()

# To make sure the "train_neural_network" function that is used in main project is working correctly.
